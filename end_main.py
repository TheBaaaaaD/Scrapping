...
for compteurD in range(1,99):
    if compteurD < 10:
        ...
        #print(baseUrl + uri + str(departement)+"/")
    ...
    if response.ok:
        ...
        liens = swoup(baseUrl + uri + str(departement)+"/",getEndpoints)
            #print(liens[i])
            #infos.append(swoup(str(liens[i]),getInfoByPage))
        print(liens)
        ...
        for link in fileReader('links.csv'):
            lignes.extend(getElements(link["lien"]))

        fields = ["nom","date","heure","adresse","typeEvenement", "departement","region","informations","contact","telephone","mail","club","siteInternet"]
        fileWriterData('infos.csv',fields,lignes)
